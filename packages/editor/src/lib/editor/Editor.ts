...
export class Editor extends EventEmitter<TLEventMap> {
	constructor({
		...
		this.sideEffects.registerAfterCreateHandler('shape', (_record) => {
	...
	createBindings = this.history.createCommand(
		'createBindings',
		(partials: RequiredKeys<TLBindingPartial, 'type' | 'toId' | 'fromId'>[]) => {
			return {
				data: {
					partials: partials.map((p) => ({ ...p, id: p.id ?? createBindingId() })),
				},
			}
		},
		{
			...
			undo: ({ partials }) => {
				this.store.remove(partials.map((partial) => partial.id))
			},
		}
	)
	...
	updateBindings = this.history.createCommand(
		'updateBindings',
		(partials: (TLBindingPartial | null | undefined)[]) => {
			const snapshots: Record<string, TLBinding> = {}
			const updates: Record<string, TLBinding> = {}
			...
			for (const partial of partials) {
				...
				const updated = applyPartialToBinding(current, partial)
				if (updated === current) continue

				snapshots[updated.id] = current
				updates[updated.id] = updated
			}

			return { data: { snapshots, updates } }
		},
		{
			do: ({ updates }) => {
				this.store.put(objectMapValues(updates))
			},
			undo: ({ snapshots }) => {
				this.store.put(objectMapValues(snapshots))
			},
			squash(prev, next) {
				return {
					// keep the oldest snapshots
					snapshots: { ...next.snapshots, ...prev.snapshots },
	...
	deleteBindings = this.history.createCommand(
		'deleteBindings',
		(bindings: (TLBinding | TLBindingId)[]) => {
			const ids = bindings.map((binding) => (typeof binding === 'string' ? binding : binding.id))
			const snapshots = ids.map((id) => this.getBinding(id)!)
			return { data: { ids, snapshots } }
		},
		{
			do: ({ ids }) => {
				this.store.remove(ids)
